...
# This script is called from the GUI to lookup the Pluto XO correction
...
timeout 2 sshpass -p analog ssh -o StrictHostKeyChecking=no root@"$PLUTOIP" 'PATH=/bin:/sbin:/usr/bin:/usr/sbin;fw_printenv xo_correction'
