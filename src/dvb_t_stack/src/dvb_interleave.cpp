#include <memory.h>
#include "dvb_t.h"

#define I_MOD  17
#define I_ROWS 12

static uint8_t d_array[2000];//where data is stored
static uint8_t *rows[I_ROWS];// data rows
static unsigned long i_mc[I_ROWS];// current row
static int i_mods[I_ROWS];//length of each row

void dvb_interleave_init( void )
{
    int i;
    int m = 0;
    rows[0] = &d_array[m];
    m++;
    for( i = 1; i < I_ROWS; i++ )
    {
        rows[i]   = &d_array[m];
        i_mods[i] = (I_MOD*i);
        m        += i_mods[i];
        i_mc[i]   = 0;
    }
    //printf("%d \n",m );
}
inline void dvb_interleave_sub( uint8_t *inout )
{
    int   i;
    int   off;
    uint8_t   out;

//	rows[0][0] = inout[0];
//	inout[0]   = rows[0][0];

    for( i = 1; i < I_ROWS; i++ )
    {
        off          = i_mc[i];
        out          = rows[i][off];
        rows[i][off] = inout[i];
        inout[i]     = out;
        // Update modulo counter
        i_mc[i]      = (i_mc[i]+1)%i_mods[i];
    }
}
//
// Do this inplace, interleave the bytes
//
void dvb_convolutional_interleave( uint8_t *inout )
{
    int i;

    for( i = 0; i < DVBS_T_CODED_FRAME_LEN; i+= I_ROWS )
    {
        dvb_interleave_sub( &inout[i] );
    }
}
