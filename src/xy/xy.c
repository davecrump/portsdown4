...
#define PATH_XYCONFIG "/home/pi/rpidatv/src/xy/xy_config.txt"
...
int CreateButton(int MenuIndex, int ButtonPosition)
{
  ...
  if ((MenuIndex != 41) && (MenuIndex != 2))   // All except keyboard and Markers
  {
...
normalised = FALSE;
  while(TRUE)
  {
    do  // Wait here for flyback
    {
      x = mcp3002_value(0);
      //printf("X value awaiting 0 = %d\n", x);
    }
    while ((x != 0) && (ContScan == FALSE));
    ...
    for (pixel = 2; pixel < 500; pixel++)  // Subsequent Samples -----------------------
    {
      ...
      while (((pixel * xscalenum) > (x * xscaleden)) && (ContScan == FALSE));
